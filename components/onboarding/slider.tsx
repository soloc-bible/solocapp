...
import index from "@/app/(routes)/onboarding";
...
const Slider = React.memo(({
  ...
  .onStart(({ x }) => {
    activeSide.value = 
      x <= MARGIN_WIDTH && hasPrev ? Side.LEFT :
      x >= WIDTH - MARGIN_WIDTH && hasNext ? Side.RIGHT : 
      Side.NONE;
    zIndex.value = activeSide.value === Side.LEFT ? 100 : 0;
  })
  .onUpdate(({ x, y }) => {
